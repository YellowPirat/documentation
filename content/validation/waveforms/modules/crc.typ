...
The time point of the comparison, shown with the red arrow in @fig:crc_example, corresponds to the time point when the crc_valid_i signal is set high. If two CRC values contain the same value, the crc_error_o isn't set high. Otherwise, the value is set high, signaling a mismatch and therefore an error.
