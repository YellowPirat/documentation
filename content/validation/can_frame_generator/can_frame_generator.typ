...
A test bench in VDHL was written to read the generated data by the CAN-Frame generator script and simulate it using GDHL. The output is a .ghw file, a waveform file, which can be viewed using a wave viewer, like GTKWAve.
...
As stated before, the generated CAN frames contain, when necessary, bit stuff bits and calculated CRC for the specific message. The different frames are then combined and saved in a single .csv file. The start of the .csv file contains a specific number of recessive bits (logical "1"), since an idle period is needed at the beginning for synchronization purposes.

Using the script, a user can test with various different CAN messages, to ensure that the CAN-Core implementation is indeed correct. However, a point already mentioned before is the implementation of the intentional errors into these generated CAN messages. This is also a crucial point in testing, since in real application, there are many factors, internal and external, that can cause errors in the transmitted messages.
...
==== Bit stuff error
Implementing a bit stuff error depends on the structure of the generated CAN message, since the script keeps track of the stuffed bits. If there are any stuffed bits, they are just removed, causing an error. If there are no stuffed bits, a position between SOF and end of CRC sequence is randomly chosen, as that part of the bit stream is coded by the method of bit stuffing. A bit at this position is then flipped.

A bit stuff error is followed immediately by an error frame, starting at the very next bit.

==== Form error
This type of error encompasses a violation of the fixed form of certain bits in a CAN message. The implementation randomly sets one of the bits of either a CRC delimiter or ACK delimiter as a dominant bit (logical "0"). With EOF bits, one random bit is set as dominant. This causes an error, since all these bits are recessive per default.
...
Another thing to consider is that checking for bit stuff error inside a CRC sequence alone is not enough. The preceding bits, in combination with the flipped bits of the CRC sequence, can cause a bit stuff error, where there wasn't one before. Therefore, additional bits are taken into consideration when checking if a bit stuff error was caused.

The position of the error frame when a CRC error is detected, is specific, in comparison to the other types of errors. It comes after the ACK delimiter, instead of immediately the next bit.